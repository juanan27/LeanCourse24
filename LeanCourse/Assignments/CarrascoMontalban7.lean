...
lemma add_pow_eq_pow_add_pow (x y : R) : (x + y) ^ p = x ^ p + y ^ p := by {
  ...
  have dvd_choose : ∀ i ∈ Ioo 0 p, p ∣ Nat.choose p i := by
    intro i hi
    simp at *
    refine Prime.dvd_choose_self hp' ?_ ?_
    <;> omega
  ...
  calc
    _ =  ∑ i in Ioo 0 p, x ^ i * y ^ (p - i) * 0 := by {
      ...
      have h0 : ∀ i ∈ Ioo 0 p, x ^ i * y ^ (p - i) * ↑(p.choose i) = 0 :=
      by exact fun i a ↦ mul_eq_zero_of_right (x ^ i * y ^ (p - i)) $ h8 i a
      exact sum_eq_zero h0
      }

    _ = 0 := by simp
  rw [add_pow]
  have h9 : ∑ m ∈ range (p + 1), x ^ m * y ^ (p - m) * ↑(p.choose m) = ∑ m ∈ range (p), x ^ m * y ^ (p - m) * ↑(p.choose m) + x ^ p * y ^ (p - p) * ↑(p.choose p) :=
  by exact sum_range_succ (fun x_1 ↦ x ^ x_1 * y ^ (p - x_1) * ↑(p.choose x_1)) p
  rw [h9, range_eq_insert_Ioo]; simp [h6]; exact AddCommMagma.add_comm (y ^ p) $ x ^ p
  }
