...
: ContinuousOn (fun z => 1/(2*Real.pi*Complex.I) * ∫ t in I, (deriv γ t) / (γ t - z)) ((univ \ (image γ I)) : Set ℂ ) := by {
  ...
  · sorry -- USAR TODO LO DE ANTES Y TERMINARLO
  · exact ?_
  · exact ?_
}
...
: ContinuousOn (fun z => 1/(2*Real.pi*Complex.I) * ∫ t in I, (deriv γ t) / (γ t - z)) ((univ \ (image γ I)) : Set ℂ ) := by {
  ...
  have hzwl : ∀ z ∈ Metric.ball z₀ ε, ‖z₀ - z‖ < d := by {
    intro z hz
    have hz_dist : dist z z₀ < ε := mem_ball.mp hz
    rw [NormedField.dist_eq] at hz_dist
    have triv : ‖z - z₀‖ = ‖z₀ - z‖ := by exact norm_sub_rev z z₀
    rw [triv] at hz_dist
    have lower_bound := hd1
    specialize hz₀w z hz
    have key_ineq : ∀ w ∈ γ.toFun '' I, d ≤ ‖z₀ - z‖ + ‖z - w‖ := by {
      intro w hw
      exact le_trans (hd1 w hw) (hz₀w w hw)
    }
    have final_bound : ∀ w ∈ γ.toFun '' I, d - ‖z - w‖ ≤ ‖z₀ - z‖ := by {
      intro w hw
      linarith [key_ineq w hw]
    }
    sorry

  }

  have hzwl' : ∀ z ∈ Metric.ball z₀ ε, ∀ w ∈ γ '' I, d ≤ d + ‖z - w‖ := by {
    intro z hz w hw
    have hzwpos : 0 ≤ ‖z - w‖ := by exact norm_nonneg (z - w)
    linarith
  }
  ...
  have midstep2 : ‖a - z₀‖ * (C / (d * (d / 2))) < ε * (C / (d * (d / 2))) := by {
    ...
    --exact (by exact haz₀)
    --field_simp
    -- FÁCIL PERO NO ENCUENTRO EL LEMA NECESARIO!!
  }
  · sorry
  · sorry
  · sorry
